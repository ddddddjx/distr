...
- 🎬 **自动慢放回放**：实时模式自动录制本次挥杆（MediaRecorder）、视频模式截取
  挥杆区间，报告中 0.4 倍速循环回放；附准备/顶点/击球/收杆四个关键位置帧
- ⏱️ **节奏（Tempo）**：自动测量上杆:下杆时长比（职业参考 3:1）
...
1. **基准采集**：球员摆好准备姿势并静止约 600ms 后，记录脊柱前倾角、头/髋/手位置、
   肩宽与躯干长度作为基准。
2. **镜头运动补偿**：所有位置都转换为"以双脚踝中点为原点、以躯干长度为单位"的
   归一化坐标——挥杆中双脚钉在地上，因此镜头平移、跟拍、变焦（电视转播类素材）
   不会造成位移误报；脚踝不可见时自动退化为画面坐标。
3. **阶段切分**：以双手中点的高度与垂直速度驱动状态机（上杆=手上升，顶点=速度反向，
   击球=手回到基准高度附近，收杆=手高于肩且静止）。
4. **方向自适应**：目标方向由上杆时手的移动方向反推，因此**与镜像、左右手、机位朝向无关**。
5. **问题检测与连续评分**：每个阶段运行对应规则，记录超出阈值的程度（ratio）；
   扣分随偏差幅度线性增长——刚擦线的轻微超标只扣 4-8 分，明显超标才扣满
   10-20 分（具体阈值见 `js/swingAnalyzer.js`，可按教学标准微调）。
...
`xhs-tool/` 是针对小红书「小工具」容器的独立版本（纯本地、无网络、
无 WASM/Worker、禁内联脚本——容器禁 AI 重计算，故本版不含姿态估计），
定位为教学笔记的互动配套：

- 慢放（0.25/0.5/1×）+ 逐帧步进回看
- 画线自查：直线 / 三点量角器（实时显示度数）/ 圆圈，红绿双色
- 冻结帧叠加对比（准备 vs 击球，透明度可调）
- 挥杆节奏计时（按视频时间轴计算，慢放不影响结果）
- TPI 十二大特征图解自查清单（本地保存主攻标记）

上架：小红书创作者中心 → Builder Hub → 小工具 → 新建 → 上传
`swingcoach-xhs-tool.zip` 内容 → 配置审核 → 移动端发笔记时通过
「挂件/组件」入口挂载。完整 AI 分析版仍在 H5 主站。
