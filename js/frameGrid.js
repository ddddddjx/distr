...
/** 采样率。12fps 是准确度与耗时的折中：快速下杆那 0.25s 仍能采到 3 帧
 *  （再低就采不到击球瞬间了），而比 15fps 少两成帧数——每帧推理是绝对大头
 *  （实测 seek 只占 18ms，推理 380–450ms）。
 *  调它会改变分数，属于口径变更；改之前先跑 tests/analyzer.test.mjs 里的
 *  「12fps 采样下仍能识别完整挥杆」。 */
export const FILE_SAMPLE_FPS = 12;
